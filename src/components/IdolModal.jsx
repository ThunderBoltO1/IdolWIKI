...
export function IdolModal({ isOpen, mode, idol, onClose, onSave, onDelete, onLike, onGroupClick, onUserClick, onSearch, onIdolClick, highlightedChanges }) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (shouldReset) {
                ...
                // Normalize formerCompanies to objects if they are strings
                if (initialData.formerCompanies && initialData.formerCompanies.length > 0) {
                    initialData.formerCompanies = initialData.formerCompanies.map(c =>
    ...
    const handleAddFormerCompany = () => {
        setFormData(prev => ({
            ...prev,
            formerCompanies: [...(prev.formerCompanies || []), { company: '', duration: '' }]
        }));
    };

    const handleUpdateFormerCompany = (index, field, value) => {
        setFormData(prev => {
            const updated = [...(prev.formerCompanies || [])];
            const current = updated[index];
            const item = typeof current === 'string' ? { company: current, duration: '' } : { ...current };
            item[field] = value;
            updated[index] = item;
            return { ...prev, formerCompanies: updated };
        });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        toast.success(`Idol "${updatedData.name}" ${mode === 'create' ? 'created' : 'updated'} successfully!`);
    ...
    return createPortal(
        <AnimatePresence>
            <div className="fixed inset-0 z-[90] flex items-center justify-center md:p-2 overflow-hidden">
                <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                ...
                <AnimatePresence>
                    {showSuccess && (
                        <motion.div
                            ...
                            className="fixed top-4 right-4 md:top-8 md:right-8 z-[110] bg-emerald-500 text-white pl-4 md:pl-6 pr-2 md:pr-3 py-2 md:py-3 rounded-2xl shadow-[0_20px_50px_-12px_rgba(16,185,129,0.5)] flex items-center justify-between gap-3 md:gap-4 font-black uppercase text-xs tracking-widest border border-white/20 backdrop-blur-xl max-w-[calc(100vw-2rem)]"
                        >
                ...
                >
                    ...
                    <button
                        onClick={onClose}
                        className={cn(
                            "absolute top-4 right-4 md:top-6 md:right-6 z-20 p-2 md:p-2.5 rounded-full transition-all duration-300 active:scale-90 shadow-lg",
                    ...
                    {history.length > 0 && !editMode && (
                        <button
                            onClick={handleBack}
                            className={cn(
                                "absolute top-4 left-4 md:top-6 md:left-6 z-20 p-2 md:p-2.5 rounded-full transition-all duration-300 active:scale-90 shadow-lg flex items-center gap-1.5 md:gap-2 pr-3 md:pr-4",
                                theme === 'dark' ? "bg-black/40 hover:bg-black/60 text-white" : "bg-white/80 hover:bg-white text-slate-800"
                            )}
                        >
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            ...
                            <div className={cn(
                                "w-full md:w-5/12 h-[30vh] min-h-[200px] md:h-auto relative flex flex-col overflow-hidden",
                                                                ...
                                                                <div className="space-y-2">
                                                                    {(formData.formerCompanies || []).map((item, index) => (
                                                                        <div key={index} className="flex gap-2">
                                                                            ...
                                                                            <button
                                                                                type="button"
                                                                                onClick={() => handleRemoveFormerCompany(index)}
                                                                                className={cn(
                                                                    ...
                                                                    >
                                                                        <Plus size={16} /> Add Former Company
                                                                    </button>
                                                                </div>
                                                            </div>
                                                            <div className="sm:col-span-2 space-y-2">
